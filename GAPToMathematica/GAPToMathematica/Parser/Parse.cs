	...
	public static partial class Parse
	{
		...
		public static Parser<char> Char(Predicate<char> predicate, string description)
		{
			...
			return i =>
			{
				if (!i.AtEnd)
				{
					if (predicate(i.Current)) return Result.Success(i.Current, i.Advance());
		...
		/// <param name="c"></param>
		/// <returns></returns> 
		public static Parser<char> CharExcept(string c) => CharExcept(c.ToEnumerable().Contains, StringExtensions.Join("|", c.ToEnumerable()));
		...
		/// <param name="c"></param>
		/// <returns></returns>
		public static Parser<char> IgnoreCase(char c) => Char(ch => char.ToLower(c) == char.ToLower(ch), char.ToString(c));
		...
		public static Parser<IEnumerable<char>> IgnoreCase(string s)
		{
			...
			return s.ToEnumerable().Select(IgnoreCase).Aggregate(Return(Enumerable.Empty<char>()), (a, p) => a.Concat(p.Once())).Named(s);
		}
		...
		{
			if (s == null) throw new ArgumentNullException("s");
			return s.ToEnumerable().Select(Char).Aggregate(Return(Enumerable.Empty<char>()), (a, p) => a.Concat(p.Once())).Named(s);
		}
		...
		public static Parser<object> Not<T>(this Parser<T> parser)
		{
			...
			return i =>
			{
				...
				return result.WasSuccessful
					? Result.Failure<object>(i, $"`{StringExtensions.Join(", ", result.Expectations)}' was not expected", new string[0])
					: Result.Success<object>(null, i);
			};
		}
		...
		public static Parser<T> Ref<T>(Func<Parser<T>> reference)
		{
			...
			return i =>
			{
				...
				if (i.Memos.ContainsKey(p)) throw new ParseException(i.Memos[p].ToString());
				i.Memos[p] = Result.Failure<T>(i, "Left recursion in the grammar.", new string[0]);
				var result = p(i);
				i.Memos[p] = result;
				return result;
			};
		...
		/// <returns></returns>
		public static Parser<string> Text(this Parser<IEnumerable<char>> characters) => characters.Select(chs => new string(chs.ToArray()));
		...
		public static Parser<T> Or<T>(this Parser<T> first, Parser<T> second)
		{
			...
			{
				var fr = first(i);
				if (!fr.WasSuccessful) return second(i).IfFailure(sf => DetermineBestError(fr, sf));
				if (fr.Remainder.Equals(i)) return second(i).IfFailure(sf => fr);
				return fr;
			};
		}
		...
		public static Parser<T> Named<T>(this Parser<T> parser, string name)
		{
			...
			return i => parser(i).IfFailure(f => f.Remainder.Equals(i) ? Result.Failure<T>(f.Remainder, f.Message, new[] { name }) : f);
		}
		...
		public static Parser<T> Except<T, U>(this Parser<T> parser, Parser<U> except)
		{
			...
			{
				var r = except(i);
				if (r.WasSuccessful) return Result.Failure<T>(i, "Excepted parser succeeded.", new[] { "other than the excepted input" });
				return parser(i);
			};
		}
		...
		/// <returns></returns>
		public static Parser<IEnumerable<T>> Until<T, U>(this Parser<T> parser, Parser<U> until) => parser.Except(until).Many().Then(r => until.Return(r));
		...
		public static Parser<T> Where<T>(this Parser<T> parser, Func<T, bool> predicate)
		{
			if (parser == null) throw new ArgumentNullException("parser");
			if (predicate == null) throw new ArgumentNullException("predicate");
			return i => parser(i).IfSuccess(s => predicate(s.Value) ? s : Result.Failure<T>(i, $"Unexpected {s.Value}.", new string[0]));
		}
		...
		static Parser<string> DecimalWithoutLeadingDigits(CultureInfo ci = null) => from nothing in Return("")
		...
		static Parser<string> DecimalWithLeadingDigits(CultureInfo ci = null) => Number.Then(n => DecimalWithoutLeadingDigits(ci).XOr(Return("")).Select(f => n + f));

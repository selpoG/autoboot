...
namespace Sprache
{
	public static partial class Parse
	{
		...
		public static Parser<Match> RegexMatch(Regex regex, string description = null)
		{
			...
			var expectations = description == null ? new string[0] : new[] { description };
			...
			return i =>
			{
				if (!i.AtEnd)
				{
					...
					if (match.Success)
					{
						for (var j = 0; j < match.Length; j++) remainder = remainder.Advance();
						return Result.Success(match, remainder);
					}
					var found = match.Index == input.Length ? "end of source" : $"`{input[match.Index]}'";
		...
		/// </remarks>
		static Regex OptimizeRegex(Regex regex) => new Regex($"^(?:{regex})", regex.Options);
	}
}

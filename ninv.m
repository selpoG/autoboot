...
Needs["ToCpp`", "tocpp.m"]
NClebschGordan`clearCG[];

BeginPackage["NClebschGordan`"]
...
precision::usage = "precision represents internal precision for calculation."
...
CommonFunctions`importPackage["CommonFunctions`", "NClebschGordan`Private`", {"importPackage", "lhs", "rhs", "MyReap", "newUF", "newSet", "classify", "keys", "add", "reverseIndex", "zero", "root", "unite", "has"}]
setGroup[G_, prec_, eps_] := myAbortProtect @ Module[{x, name = "NClebschGordan`Private`"},
	Unprotect[symmetryGroup, one];
	symmetryGroup = G;
	precision = prec; epsilon = eps;
...
Module[{sol, a, b, c, d = dim[r], d3 = dim[t], l, n, sym, tmp, v, p, even, odd},
	sym[v_, p_] := (
		...
		Expand[tmp]);
	l = Length[sol = NullSpace @ num @ eq[r, r, t]];
	If[l == 0, Message[setOPE::imcmpt, r, r, t]; Return[]];
	If[l != inv[{r, r}, {t}], Message[setOPE::diff, r, r, t, l, inv[{r, r}, {t}]]; Return[]];
	even = Select[num @ Orthogonalize[num @ sym[#,  1] & /@ sol, Conjugate[#1].#2 &], AnyTrue[#, # != 0 &] &];
	odd  = Select[num @ Orthogonalize[num @ sym[#, -1] & /@ sol, Conjugate[#1].#2 &], AnyTrue[#, # != 0 &] &];
	l = Length[even];
	myAbortProtect[
...
ope[r_, i_, r_][1][a_, 1, b_] /; i === id := num @ If[a == b, 1, 0]
ope[i_, r_, r_][1][1, a_, b_] /; i === id := num @ If[a == b, 1, 0]
x:ope[r_, s_, t_][n_][a_, b_, c_] /; 1 <= n <= inv[{r, s}, {t}] := (setOPE[r, s, t]; x)
ope[_, _, _][_][_, _, _] := 0

x:setOPE[r_] := x = Module[{r2 = dual[r], a, b, d = dim[r], sd},
	sd = Sqrt[d]; myAbortProtect @ Do[ope[r][a, b] = sd ope[r, r2, id][1][a, b, 1], {a, d}, {b, d}]]
x:ope[r_][a_, b_] := (setOPE[r]; x)

x:setCor[r_, s_, t_] := x = Module[{n, a, b, c, c2, d = dim[t], sd, t2 = dual[t], res},
	sd = Sqrt[d];
	Do[res[n, a, b, c] = num @ Sum[ope[r, s, t2][n][a, b, c2] ope[t2][c2, c], {c2, d}] / sd
...
x:setCor[r1_, r2_, r3_, r4_, s_] := x = Module[{n, m, a1, a2, a3, a4, s2 = dual[s], res},
	Do[res[n, m, a1, a2, a3, a4] = num @ Sum[cor[r1, r2, s2][n][a1, a2, b] ope[r3, r4, s2][m][a3, a4, b], {b, dim[s]}]
...
dec[r_, s_, t_][f_] := Module[{vec},
	decPrep[r, s, t]; vec = Array[{f @@ bas[r, s, t][#]} &, inv[r, s, t]]; num @ Flatten[invmat[r, s, t].vec]]
...
Module[{n, f = cor[r, s, t], max = inv[r, s, t], mat, rev, new, old, sc, a, b, c, tmp, tmp2, res},
	Do[If[f[1][a, b, c] != 0, res[1] = {a, b, c}; mat = {{num @ f[1][a, b, c]}}; rev = Inverse[mat]; Break[]]
	...
	Do[Do[
		...
		sc = f[n][a, b, c];
		tmp2 = num[rev.new];
		If[(tmp = num[sc - (old.tmp2)[[1, 1]]]) != 0,
			res[n] = {a, b, c};
			mat = ArrayFlatten[{{mat, new}, {old, {{sc}}}}];
			rev = num @ ArrayFlatten[{{rev + tmp2.old.rev / tmp, -tmp2 / tmp}, {-old.rev / tmp, {{1 / tmp}}}}];
			Break[]]
...
getBasis[f_, ks_, args_] :=
Module[{mat, rev, new, old, sc, a, k, res, tmp, tmp2},
	Do[
		If[(new = (f @@ ks[[1]]) @@ a) != 0, res[1] = a; mat = {{new}}; rev = Inverse[mat]; Break[]]
	, {a, args}];
	Do[Do[
		k = ks[[n]];
		new = Array[{(f @@ k) @@ res[#]} &, n - 1];
		old = {Array[(f @@ ks[[#]]) @@ a &, n - 1]};
		sc = (f @@ k) @@ a;
		tmp2 = num[rev.new];
		If[(tmp = num[sc - (old.tmp2)[[1, 1]]]) != 0,
			res[n] = a;
...
simpl[x_] := simpl2[Expand[x]]
simpl2[0] = 0;
simpl2[x_Plus] := simpl2 /@ x
simpl2[x_sum] := x
simpl2[x_single] := x
simpl2[x_?NumericQ y_] := Chop[N[x, precision - 5], epsilon * 10^5] y
...
rotate[Ls_List] := Module[{N = Length[Ls], n, m, r, s, t, x = List @@ Take[Ls[[1, 1]], 3], tau, o1, o2, o3, sg},
	...
	tau = \[Tau][t, r, s];
	Table[L[L[o3, o1, o2, n], sg Sum[tau[n, m] Ls[[m, 2]], {m, N}]], {n, N}]
]
flipim[Ls_List] := Module[{N = Length[Ls], n, m, r, s, t, x = List @@ Take[Ls[[1, 1]], 3], omega, o1, o2, o3, d1, d2, d3, sg},
	...
	sg = \[Sigma][o1 o2 d3];
	omega = \[Omega][r, s, t];
	Table[L[L[d1, d2, d3, n], ComplexExpand[sg Sum[omega[n, m] Conjugate[Ls[[m, 2]]], {m, N}]]], {n, N}]
]
...
setOPE[o1 : op[_, r_, 1 | -1, 1], o2 : op[_, s_, 1 | -1, 1], o3 : op[_, t_, 1 | -1, 1 | -1]] :=
	Module[{re, im, a, b, c, d, cnt, n, N = inv[r, s, t], sol, add, eq, ev0, res},
		...
		sol = NullSpace @ If[eq === {}, {ConstantArray[0, 2 N]}, num @ eq];
		sol = Array[\[Beta][o1, o2, o3][#] &, Length[sol]] . sol;
		If[sol === 0, sol = ConstantArray[0, 2 N]];
		Do[re[n] = sol[[n]], {n, N}];
		Do[im[n] = sol[[n + N]], {n, N}];
		ev0[L[a_, b_, c_, n_]] := res[a, b, c, n] = cnt[L[a, b, c, n]][[1, 2]];
...
boot[o1 : op[_, r1_, 1 | -1, 1], o2 : op[_, r2_, 1 | -1, 1], o3 : op[_, r3_, 1 | -1, 1], o4 : op[_, r4_, 1 | -1, 1]][s_, n_, m_, p : 1 | -1] :=
	Module[{t, k, l}, 0 == simpl[sF2[o1, o2, o3, o4][s, n, m, p]
...
boot[o1 : op[_, r1_, 1 | -1, 1], o2 : op[_, r2_, 1 | -1, 1], o3 : op[_, r3_, 1 | -1, 1], o4 : op[_, r4_, 1 | -1, 1]][s_] :=
	reim @ MyReap @ Module[{n, m, p}, Do[Sow[boot[o1, o2, o3, o4][s, n, m, p]], {n, inv[{r1, r2}, {s}]}, {m, inv[r3, r4, s]}, {p, {1, -1}}]]
bootAll[ops_List] := eqn @ reduceBoot @ reim @ MyReap @ Module[{a, b, c, d, s}, Do[Sow[boot[a, b, c, d][s]], {a, ops}, {b, ops}, {s, prodOp[a, b]}, {c, ops}, {d, ops}]]
bootAll[] := bootAll @ keys @ allopsForBoot

reduceBoot[True] = {};
reduceBoot[x_ == y_] := {Expand[x - y]}
reduceBoot[eq_And] := Module[{add, bls = <||>, ks, scan, scan2, mat = <||>, row, n, b, min = Infinity, max = 0},
	scan[x_, n_] := scan2[#, n]& /@ x;
	scan2[Times[x_?NumericQ, y_], n_] := (add[y]; mat[n][bls[y]] = x);
	add[x:(_sum|_single)] := If[!KeyExistsQ[bls, x], bls[x] = 1 + Length[bls]];
	Do[mat[n] = <||>; scan[Expand[eq[[n, 1]] - eq[[n, 2]]], n], {n, Length[eq]}];
	mat = Table[If[KeyExistsQ[mat[n], b], mat[n][b], 0], {n, Length[eq]}, {b, Length[bls]}];
	mat = Select[num @ RowReduce[mat], AnyTrue[#, # != 0 &] &];
	Do[If[x =!= 0, min = Min[min, Abs[x]]; max = Max[max, Abs[x]]];, {x, Flatten[mat]}]; Print[{min, max}];
	ks = Keys[bls];
	Table[Sum[ks[[n]] row[[n]], {n, Length[ks]}], {row, mat}]
]
...
makeSDP[z:eqn[eq_List]] := Module[{mat, tmp, sec = sector[z], secs = <||>, scalnum, val = newSet[], f, s, i, add2},
		add2[x_] := If[num @ Abs[x - 1] >= 10^21 epsilon, add[val, x]];
		f[x_Plus] := f /@ x;
		f[0] = 0;
		f[Times[x_?NumericQ, y_]] := If[x > 0, add2[x]; block[1, x, y], add2[-x]; block[-1, -x, y]];
		f[y_] := block[1, 1, y];
...
toQboot[sdpobj[secs_, scalarnum_, vals_, mat_]] :=
	Module[{s, v, revval = reverseIndex[vals], numeq = Length @ mat[unit, 1][[2]], secsStr, valsStr, filename, extdelta, exts, extops, scalarsecs, secname, matsize, terms, syms, n, i, e, r, c, f, regsym, eqStr, convert},
		...
		(* diagonal *)
		convert[0, block[1, one_, bl_]] /; num[Abs[one - 1]] < 10^21 epsilon := toCString[bl];
		convert[0, block[-1, one_, bl_]] /; num[Abs[one - 1]] < 10^21 epsilon := TemplateApply["real(-1), `bl`", <|"bl" -> toCString[bl]|>];
		convert[0, block[1, v_, bl_]] := TemplateApply["val[`i`], `bl`", <|"i" -> revval[v], "bl" -> toCString[bl]|>];
		convert[0, block[-1, v_, bl_]] := TemplateApply["-val[`i`], `bl`", <|"i" -> revval[v], "bl" -> toCString[bl]|>];
		(* off-diagonal *)
		convert[1, block[1, one_, bl_]] /; num[Abs[one - 1]] < 10^21 epsilon := TemplateApply["real(2), `bl`", <|"bl" -> toCString[bl]|>];
		convert[1, block[-1, one_, bl_]] /; num[Abs[one - 1]] < 10^21 epsilon := TemplateApply["real(-2), `bl`", <|"bl" -> toCString[bl]|>];
		...
		Do[f[scalar, n, e, r, c, mat[scalar, n][[2, e, r, c]]], {n, scalarnum}, {e, numeq}, {r, matsize[scalar, n]}, {c, r, matsize[scalar, n]}];
		Do[f[s, n, e, r, c, mat[s, n][[2, e, r, c]]], {s, Keys[secs]}, {n, secs[s]}, {e, numeq}, {r, matsize[s, n]}, {c, r, matsize[s, n]}];
...
toCboot[sdpobj[secs_, scalarnum_, vals_, mat_]] :=
	Module[{s, v, revval = reverseIndex[vals], blk, f, convert, make, tmp, secsStr, valsStr, rmats, smats, umats, filename, opinfo},
		...
		convert[0] = "0";
		convert[block[1, one_, bl_]] /; num[Abs[one - 1]] < 10^21 epsilon := TemplateApply["bl[`i`]", <|"i" -> blk[bl]|>];
		convert[block[-1, one_, bl_]] /; num[Abs[one - 1]] < 10^21 epsilon := TemplateApply["-bl[`i`]", <|"i" -> blk[bl]|>];
